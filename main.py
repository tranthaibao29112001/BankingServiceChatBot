...
class Converter(object):
    ...
    def convert_csv_to_yaml(self, rasa_version: str = "3.0"):
        ...
        for intent in intents:
            ...
            intent_texts = filter(lambda x: x.lower().find("how") != -1 and x.lower().find("?") != -1, intent_texts)
            #intent_texts = filter(lambda x: x.lower().find("what") != -1 and x.lower().find("?") != -1, intent_texts)
...
if __name__ == '__main__':
    convert('C:\\Users\\DELL\\Desktop\\test.csv', 'intent', 'text', 'examples', '3.0', '.', 'temp-nlu.yml', False)